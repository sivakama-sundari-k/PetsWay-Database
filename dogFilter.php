...
$insert=false;
 ...
 $sql="SELECT * from `petswaydatabase`.`pets` WHERE `petswaydatabase`.`pets`.petType='Dog' AND NOT `petswaydatabase`.`pets`.petStatus='sold'";

     if (isset($_POST['petAge'])){
          $petAge=$_POST['petAge'];
            $sql=$sql . " AND `petswaydatabase`.`pets`.petAge=$petAge";
        
    }

    if (isset($_POST['petPrice'])){
          $petPrice=$_POST['petPrice'];
            $sql=$sql . " AND `petswaydatabase`.`pets`.petPrice<=$petPrice";
    }
   
    
    if (isset($_POST['petGender'])){
          $petGender=$_POST['petGender'];
            $sql=$sql . " AND `petswaydatabase`.`pets`.petGender='$petGender'";
    }
    if (isset($_POST['petBreed'])){
          $petBreed=$_POST['petBreed'];
            $sql=$sql . " AND `petswaydatabase`.`pets`.petBreed='$petBreed'";
            
    }
    if (isset($_POST['petCategoryName'])){
          $petCategoryName=$_POST['petCategoryName'];
            $sql="SELECT `petswaydatabase`.`pets`.petID, `petswaydatabase`.`pets`.petCategoryID, `petswaydatabase`.`pets`.petType, `petswaydatabase`.`pets`.petBreed,`petswaydatabase`.`pets`.petGender, `petswaydatabase`.`pets`.petName,`petswaydatabase`.`pets`.petAge, `petswaydatabase`.`pets`.petPrice,`petswaydatabase`.`pets`.petImage, `petswaydatabase`.`pets`.petDesc, `petswaydatabase`.`pets`.petStatus FROM `petswaydatabase`.`pets`,`petswaydatabase`.`petcategories` WHERE `petswaydatabase`.`pets`.petCategoryID=`petswaydatabase`.`petcategories`.petCategoryID AND `petswaydatabase`.`petcategories`.petCategoryName='$petCategoryName' INTERSECT " .$sql;
          
    }
    if(!isset($_POST['petAge']) && !isset($_POST['petPrice']) && !isset($_POST['petGender']) && !isset($_POST['petBreed']) && !isset($_POST['petCategoryName']) )
    {
        header("location:dogresults.php");
        exit();
    }
   ...
   <div style="text-align: -webkit-center; word-spacing: 429px;margin-top: -5px;
    ...
    opacity: 0.9;">
     ...
     <div style="display: inline-block; word-spacing:300px; margin-left:46px;margin-right:46px; margin-top:100px;">
        ...
        <img src="petImage\<?php echo $rows["petImage"]; ?>" class="image" style="height: 300px; width: 250px; display: inline-block;" alt="image">
        <!-- <h1 style="color:white;"> <?php echo $rows["petName"]; ?></h1> -->
        <input style="width:0%;" type="hidden" name="petID" value= <?php echo $rows["petID"]; ?>>
        <?php if($rows["petStatus"]!='sold'){ ?>
            <div class="middle">
