...
<div class="container" style="align-items: center; text-align: center; align-content: center;">
    ...
    <form action="addcatproductcode.php" method="post" id="form" enctype="multipart/form-data">
        
        <label for="productName">Name: </label>
        <input type="text" name="productName" id="productName" placeholder="Enter product's name here"  required> <br>
            <!-- for image upload -->
        ...
        <input type="number" name="availableQuantity" id="availableQuantity" placeholder="Enter product quantity here"  required> <br>
        <label for="productDesc">Description: </label>
        <input type="text" name="productDesc" id="productDesc" placeholder="Enter product's description here"  required> <br>
        <label for="productCost">Cost: </label>
        <input type="text" name="productCost" id="productCost" placeholder="Enter product cost here" required> <br>
    ...
    margin-bottom: 18px;" required>
            <option disabled value="" selected hidden style ="color:black;">Select product category</option> 
            <?php
    ...
    if(!$con){
        die("connection to this  database failed due to " . mysqli_connect_error());
            ...
            $sql2="SELECT * FROM `petswaydatabase`.`productcategories` WHERE `petswaydatabase`.`productcategories`.petType='Cat';";
            $res2=$con->query($sql2);
            while($rows=mysqli_fetch_array($res2,MYSQLI_ASSOC)){
                ?>
                    <option value="<?php echo $rows["productCategoryID"]; ?> " style ="color:black;"> ID: <?php echo $rows["productCategoryID"]; ?> - Name: <?php echo $rows["productCategoryName"]; ?></option>
            <?php
            }//end of while
            ?>
        ...
        <button type="submit" class = "register" style="margin-left: 101px;
    margin-top: 13px;
    padding: 5px;
