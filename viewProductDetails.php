...
if(isset($_POST['productID'])){
    ...
    $sql="SELECT `petswaydatabase`.`products`.productImage,`petswaydatabase`.`products`.productName,`petswaydatabase`.`products`.productDesc, `petswaydatabase`.`products`.productCost, `petswaydatabase`.`products`.productName FROM `petswaydatabase`.`products` WHERE `petswaydatabase`.`products`.productID='$productID';";
...
<div style =" word-spacing: 7px;
  ...
  <div class="grid-item" style="background-color: #0d0d0d;
    text-align: center;">
    <img style ="width:250px; height:300px;" src="productImage\<?php echo $row["productImage"]; ?>" style=""alt="image">
</div>
<form action="buyProduct.php" method="post">
  <div class="grid-item" style="background-color: whitesmoke;
    ...
    padding-top: 40px; padding-bottom: 40px;padding-right: 58px;">
      <h1 style="text-align: center; opacity: 50%;"><?php echo $row["productName"]; ?></h1>
    
    <p> Description: <?php echo $row["productDesc"]; ?> </p>
    <p style="-webkit-text-stroke: thin;
    font-size: larger;"> Price: Rs. <?php echo $row["productCost"]; ?> </p>
    <input style="width:0%;" type="hidden" name="productID" value= <?php echo $productID; ?>>
