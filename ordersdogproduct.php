...
<head>
    ...
    <title>Dog Products Orders</title>
...
$sql="SELECT `petswaydatabase`.`productorders`.productOrderID,`petswaydatabase`.`productorders`.productOrderDate, `petswaydatabase`.`productorders`.productID, `petswaydatabase`.`productcategories`.productCategoryName, `petswaydatabase`.`products`.productCost,`petswaydatabase`.`productorders`.custID, `petswaydatabase`.`customernew`.custFirstName,`petswaydatabase`.`customernew`.custLastName,`petswaydatabase`.`customernew`.custContactNum FROM `petswaydatabase`.`productorders`, `petswaydatabase`.`products`,`petswaydatabase`.`customernew`,`petswaydatabase`.`productcategories` WHERE (`petswaydatabase`.`customernew`.custID = `petswaydatabase`.`productorders`.custID AND `petswaydatabase`.`productorders`.productID = `petswaydatabase`.`products`.productID AND `petswaydatabase`.`products`.productCategoryID=`petswaydatabase`.`productcategories`.productCategoryID AND `petswaydatabase`.`productcategories`.petType='Dog');";

// INSERT INTO `petorders` (`petOrderID`, `petOrderDate`, `custID`, `petID`) VALUES ('101',//'2020-08-15', '1', '2');
...
<?php
    ...
    ?>
    <tr>
            <td><?php echo $rows["productOrderID"]; ?></td>
            <td><?php echo $rows["productOrderDate"]; ?></td>
