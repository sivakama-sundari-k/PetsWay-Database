...
<?php
    ...
    $sql = "SELECT * FROM `petswaydatabase`.`pets` WHERE `petType`='Cat';";
...
<div class="container" style="align-items: center; text-align: center; align-content: center;">
    ...
    <form action = "archivecatcode.php " method = "post">
        <label for="petID">Pet ID: </label>
        <select name="petID" id="petID" style="width: -webkit-fill-available; color:black; padding: 12px;background-color:white;
    margin-top: 11px;
    font-size: inherit;
    margin-bottom: 18px;" required> <option disabled value="" selected hidden style ="color:black;"> Say bye to </option>
        <?php
        if($insert==true){
            ...
            while($rows=mysqli_fetch_array($result,MYSQLI_ASSOC)){
                ?>
                    <option value="<?php echo $rows["petID"]; ?> " style ="color:black;">ID: <?php echo $rows["petID"]; ?> - Name: <?php echo $rows["petName"]; ?></option>
            <?php
            }//end of while
            ?>
            </select>
            <button type="submit" class = "register" style="margin-left: 120px; margin-top: 9px;"> Archive <span class="material-icons">remove_circle</span></button>
    </form>
    </div>
