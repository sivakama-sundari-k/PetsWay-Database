...
if(isset($_POST['productID'])){
              ...
              $sql2="SELECT `petswaydatabase`.`productcategories`.productCategoryID,`petswaydatabase`.`productcategories`.productCategoryName FROM `petswaydatabase`.`productcategories` WHERE `petswaydatabase`.`productcategories`.petType='Cat' ;";
            $res2=$con->query($sql2);
}
else{
    echo '<script> alert("Please select a product to be modified!") </script>';
    include 'archivecatproduct.php';
    exit();
}
?>
              <!DOCTYPE html>
              ...
              <head>
                  ...
                  <title>Update Cat Products</title>
...
<div style =" word-spacing: 7px;
    ...
    font-size: xx-large;
    margin-top: 95px;"> 
         Modifying the details of a Cat Product
   
    </div>
              <div class="container" style="align-items: center; text-align: center; align-content: center;">
            <form action = "archivecatproductcode2.php " method = "post">
