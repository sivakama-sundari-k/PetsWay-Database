...
<head>
    ...
    <title>Modify a Cat Product</title>
...
<div style =" word-spacing: 7px;
    ...
    font-size: xx-large;
    margin-top: 95px;"> 
         Modify a Cat Product
   
    </div>
<div class="container" style="align-items: center; text-align: center; align-content: center;">
    
    
    <form action = "archivecatproductcode.php" method = "post">
        <label for="productID">Product ID: </label> <br>
        <select name="productID" id="productID" style="color:black; padding: 12px;background-color:white;
    margin-top: 11px;
    font-size: inherit;
    margin-bottom: 18px;" required> 
    <option disabled value="" selected hidden style ="color:black;"> Product details to be modified </option>
        <?php
        if($insert==true){
            ...
            while($rows=mysqli_fetch_array($result,MYSQLI_ASSOC)){
                ?>
                    <option value="<?php echo $rows["productID"]; ?>" style ="color:black;"> ID: <?php echo $rows["productID"]; ?> - Name: <?php echo $rows["productName"]; ?> - Category: <?php echo $rows["productCategoryName"]; ?></option>
            <?php
            }//end of while
            ...
            <button type="submit" class = "register" style="margin-left: 276px; margin-top: 9px;"> Modify</button>
    </form>
    </div>
