...
if(isset($_POST['CardNumber'])){
    ...
    $sqlinsert="INSERT INTO `petswaydatabase`.`productorders` (`productID`,`custID`) VALUES ('$productID', '$custID');";

    $resultinsert=$con->query($sqlinsert);
    if($resultinsert){
    $sqlupdate="UPDATE `petswaydatabase`.`products` SET `petswaydatabase`.`products`.availableQuantity=(`petswaydatabase`.`products`.availableQuantity)-1 WHERE `petswaydatabase`.`products`.productID=$productID;";
    $resultupdate=$con->query($sqlupdate);
    $sql="SELECT * from `petswaydatabase`.`products` where productID=$productID;";
    $result=$con->query($sql);
    $rows=mysqli_fetch_array($result,MYSQLI_ASSOC);
    if($rows["availableQuantity"]==0){
       $sqlsold="UPDATE `petswaydatabase`.`products` SET `petswaydatabase`.`products`.productStatus='sold'  WHERE `petswaydatabase`.`products`.productID=$productID;"; 
          $resultsold=$con->query($sqlsold);
    }
...
?>
        <h6> Order placed successfully! Thank you for shopping with us. </h6>
    <?php } ?>
